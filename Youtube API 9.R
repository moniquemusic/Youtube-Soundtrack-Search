...
#for-loop to find the 5 best segments per game
for(gg_g in gg_games){
  gg2<- gg_g
  ...
  for(title in title_number){
    ...
    #set the max length of segment to 20
    #if the length of title is shorter than 20, reduce segment_lengthh to the number of characters of the title 
    if(nchar(titles2_df[,gg2][title2]) >= 12){
      segment_lengthh<- c(8:12)
    }else{
      segment_lengthh <- c(8: nchar(titles2_df[,gg2][title2]))
    }
    ...
    for(lengthh in segment_lengthh){
      ...
      #make sure the segment doesn't start too late in the title and has room for the full length of the term to fit on the end
      segment_startt<- c(1: (nchar(titles2_df[,gg2][title2]) - lengthh2))
  ...
  #remove segments that are 0 characters in length (ie: "")
  if(is.na(table(grepl(0, nchar(segment_collection)))["TRUE"]) != TRUE){
  ...
  sorted_match_spots<- sort(num_of_exact_matches, decreasing = TRUE)
  
  #create empty list to store the reuslts of the for-loop into
  sorted_segments<-c()
  
  for(sorted in sorted_match_spots){
    test<- grep(sorted, num_of_exact_matches)
    
    sorted_segments <- append(sorted_segments, segment_collection[test])
  }
  
  #sorted_match_spots<- unique(sort(num_of_exact_matches, decreasing = TRUE))
  ...
  #for(sorted in sorted_match_spots){
    ...
    #same_match_level_segments <- unique_segment_collection[test]
  ...
  while(n<= 5){
    ...
    if(is.na(table(grepl(segment, all_titles3, fixed = TRUE))["TRUE"]) == FALSE){
      ...
      if(table(grepl(segment, all_titles3, fixed = TRUE))["TRUE"] <= 200){
        best_segments_2<- append(best_segments_2, segment)
        n <- n + 1
      }
    }
    c <- c +1
...
for(gg_g in gg_games){
  ...
  used_key_terms <- best_segments_2[which_key_term]
